...
int main(int argc, char **argv)
{
    ...
    char *message = "Welcome. You have reached the server. Ctrl-C to quit.";  //welcome message
    ...
    int port_num = atoi(argv[1]);  //port number to listen on
    ...
    while (1){  //do forever
        ...
        for (i = 0; i < MAX_CLIENTS; i++){
            ...
            if (FD_ISSET(sd, &readfds)){
                nameval = recv(sd, client_name, sizeof(client_name)-1, 0);          //read name of sender
                msgval = recv(sd, buffer, sizeof(buffer)-1, 0);                   //read incomming message
                ...
                if (msgval == 0 || nameval == 0){                           //check for a disconnection
